...
import plugin from '../../../build/js/packages/@fusionpowered/api-generator/kotlin/index';
...
let catalogDir;
const openAPIExamples = join(__dirname, '../resources/openapi');
const asyncAPIExamples = join(__dirname, '../resources/asyncapi');
...
describe('OpenAPI EventCatalog Plugin', () => {
    ...
    describe('service generation', () => {
        ...
        describe('services', () => {
            ...
            it('when the OpenaPI service is already defined in EventCatalog and they are different, throw exception', async () => {
            ...
            it('when the OpenAPI service is already defined in EventCatalog and the versions match, manually added `sends` items of messages are persisted, as the plugin does not create them', async () => {
                // Create a service with the same name and version as the OpenAPI file for testing
                const {writeService, getService} = utils(catalogDir);
                ...
                await writeService(
                    {
                        ...
                        version: '1.0.0',
                        summary: 'This is a sample server Petstore server.',
                        schemaPath: 'petstore.yml',
                        badges:[{content:'Pets', backgroundColor:'blue', textColor:'blue'}],
                        specifications: { openapiPath: 'petstore.yml' },
            ...
            it('if the OpenAPI service is already defined in the EventCatalog and the versions match, the owners and repository are persisted', async () => {
                // Create a service with the same name and version as the OpenAPI file for testing
                const {writeService, getService} = utils(catalogDir);
        ...
        describe('messages', () => {
            ...
            describe('schemas', () => {
                ...
                it('when a message has a request body, the markdown contains the request body', async () => {
                    ...
                    expect(command.markdown).toContain(`## POST(/pets)
...
- **limit** (query): How many items to return at one time (max 100)`);
                ...
                it('when a message has circular references, the plugin adds [Circular] to the schema', async () => {
                    await plugin(config, {
                        services: [{path: join(openAPIExamples, 'circular-ref.yml'), id: 'circular-ref-service'}],
                    });
  ...
  describe('service generation', () => {
    ...
    describe('domains', () => {
      it('if a domain is defined in the AsyncAPI file but the versions do not match, the existing domain is version and a new one is created', async () => {
        ...
        await plugin(config, {
          services: [{ asynapiPath: join(asyncAPIExamples, 'simple.asyncapi.yml'), id: 'account-service' }],
          domain: { id: 'orders', name: 'Orders Domain', version: '1.0.0' },
        });
      ...
      it('if multiple asyncapi files are processed, they are all added to the domain', async () => {
        ...
        await plugin(config, {
          services: [
            { asynapiPath: join(asyncAPIExamples, 'simple.asyncapi.yml'), id: 'account-service' },
            { asynapiPath: join(asyncAPIExamples, 'orders-service.asyncapi.yml'), id: 'orders-service' },
          ],
          domain: { id: 'orders', name: 'Orders', version: '1.0.0' },
        });
    ...
    describe('services', () => {
      it('asyncapi is mapped into a service in EventCatalog when no service with this name is already defined', async () => {
        ...
        await plugin(config, { services: [{ asynapiPath: join(asyncAPIExamples, 'simple.asyncapi.yml'), id: 'account-service' }] });
      ...
      describe('sends', () => {
        ...
        it('[AsyncApi 2.X] any message with the operation `publish` is added to the service. The service sends this message.', async () => {
          ...
          await plugin(config, {
            services: [{ asynapiPath: join(asyncAPIExamples, 'simple.asyncapi-v2.yml'), id: 'account-service' }],
          });
      ...
      it('if the service already has specifications attached to it, the asyncapi spec file is added to this list', async () => {
        ...
        await writeService({
          ...
          name: 'Random Name',
          markdown: 'Here is my original markdown, please do not override this!',
          specifications: { path: 'simple.openapi.yml' },
        });
      ...
      it('if the service already has specifications attached to it including an AsyncAPI spec file the asyncapi file is overriden', async () => {
        ...
        await writeService({
          ...
          name: 'Random Name',
          markdown: 'Here is my original markdown, please do not override this!',
          specifications: { path: 'simple.openapi.yml', asyncapiPath: 'old.asyncapi.yml' },
        });
      ...
      it('if the service has owners, these owners are written to the service', async () => {
        ...
        await plugin(config, {
          services: [
            { asynapiPath: join(asyncAPIExamples, 'simple.asyncapi.yml'), id: 'account-service', owners: ['John Doe', 'Jane Doe'] },
          ],
        });
    ...
    describe('generator options', () => {
      describe('config option: id', () => {
        it('if an `id` value is given with the service, then the generator uses that id and does not generate one from the title', async () => {
          ...
          await plugin(config, {
            services: [{ asynapiPath: join(asyncAPIExamples, 'simple.asyncapi.yml'), id: 'account-service', id: 'custom-id' }],
          });
      ...
      describe('config options', () => {
        it('[name] if the `name` value is given in the service config options, then the service name is set to the config value', async () => {
          ...
          await plugin(config, {
            services: [
              {
                path: join(asyncAPIExamples, 'simple.asyncapi.yml'),
                id: 'account-service',
    ...
    describe('channels', () => {
      describe('when `channels` is set to true in the generator configuration file', () => {
        it('all channels in the AsyncAPI file are documented in EventCatalog', async () => {
          ...
          await plugin(config, {
            services: [{ asynapiPath: join(asyncAPIExamples, 'streetlights-kafka-asyncapi.yml'), id: 'streetlights-service' }],
            parseChannels: true,
          });
    ...
    describe('$ref', () => {
      it('AsyncAPI files with $ref are resolved and added to the catalog', async () => {
        ...
        await plugin(config, { services: [{ asynapiPath: join(asyncAPIExamples, 'ref-example.asyncapi.yml'), id: 'test-service' }] });
      ...
      it('if `parseSchemas` is false, the schemas for the AsyncAPI document are not parsed, and the original AsyncAPI file is saved against the service', async () => {
        ...
        await plugin(config, {
          services: [
            { asynapiPath: join(asyncAPIExamples, 'asyncapi-with-avro-expect-not-to-parse-schemas.yml'), id: 'test-service' },
          ],
          saveParsedSpecFile: true,
          parseSchemas: false,
        });
      ...
      it('if the AsyncAPI has any $ref these are not saved to the service. The servive AsyncAPI is has no $ref', async () => {
        await plugin(config, {
          services: [{ asynapiPath: join(asyncAPIExamples, 'ref-example.asyncapi.yml'), id: 'Test Service' }],
          saveParsedSpecFile: true,
        });
        ...
        const expected = (await fs.readFile(join(asyncAPIExamples, 'ref-example-without-refs.asyncapi.yml'))).toString();
    ...
    describe('asyncapi files with avro schemas', () => {
      it('parses the AsyncAPI file with avro schemas, and stores the avro schema against the event', async () => {
        ...
        await plugin(config, {
          services: [{ asynapiPath: join(asyncAPIExamples, 'asyncapi-with-avro.asyncapi.yml'), id: 'user-signup-api' }],
        });
    ...
    describe('AsyncAPI files as JSON', () => {
      it('parses the JSON spec file and writes the schema as JSON to the given service ', async () => {
        ...
        await plugin(config, { services: [{ asynapiPath: join(asyncAPIExamples, 'example-as-json.json'), id: 'user-service' }] });
    ...
    it('when a message has no payload, the message is still parsed and added to the catalog', async () => {
      const { getEvent } = utils(catalogDir);

      await plugin(config, {
        services: [{ asynapiPath: join(asyncAPIExamples, 'asyncapi-without-payload.yml'), id: 'my-service' }],
      });
